...
static string CreateString(size_t len);
...
string SubString(string s, size_t p1, size_t p2)
{
    size_t len;
    string result;
...
string ConvertToLowerCase(string s)
{
    ...
    for (i = 0; s[i] != '\0'; i++) result[i] = (char)tolower(s[i]);
    result[i] = '\0';
    return (result);
}
...
string ConvertToUpperCase(string s)
{
    ...
    result = CreateString(strlen(s));
    for (i = 0; s[i] != '\0'; i++) result[i] = (char)toupper(s[i]);
    result[i] = '\0';
    return (result);
}
